...
type Service interface {
	...
	RemoveMetadata(context.Context, *RemoveMetadataPayload) (err error)
}
...
// MethodKey key.
var MethodNames = [7]string{"list", "upload", "read", "addCollection", "removeCollection", "addMetadata", "removeMetadata"}
...
type ListPayload struct {
	...
	// of all Airports.
	Select string
	// DEPRECATED: List offset
	Offset *int
	// DEPRECATED: Max. number of records to return
	Limit *int
...
// RemoveMetadataPayload is the payload type of the artifact service
// removeMetadata method.
type RemoveMetadataPayload struct {
	// Artifact ID
	ID string
	// Schema of metadata to remove.
	Schema string
	// JWT used for authentication
	JWT string
}
...
type UploadPayload struct {
	...
	// comma-separated key-value pairs. The key and value MUST be separated by a
	// space.
	...
	// value MAY be empty.
	// In these cases, the space, which would normally separate the key and the
	// value, MAY be left out.
...
func newArtifactStatusRT(vres *artifactviews.ArtifactStatusRTView) *ArtifactStatusRT {
	...
	if vres.Metadata != nil {
		...
		for i, val := range vres.Metadata {
			res.Metadata[i] = transformArtifactviewsParameterTViewToParameterT(val)
		}
	}
	if vres.Account != nil {
...
func newArtifactStatusRTView(res *ArtifactStatusRT) *artifactviews.ArtifactStatusRTView {
	...
	if res.Metadata != nil {
		vres.Metadata = make([]*artifactviews.ParameterTView, len(res.Metadata))
		for i, val := range res.Metadata {
			vres.Metadata[i] = transformParameterTToArtifactviewsParameterTView(val)
		}
	}
	if res.Account != nil {
...
// transformArtifactviewsParameterTViewToParameterT builds a value of type
// *ParameterT from a value of type *artifactviews.ParameterTView.
func transformArtifactviewsParameterTViewToParameterT(v *artifactviews.ParameterTView) *ParameterT {
	if v == nil {
		return nil
	}
	res := &ParameterT{
...
// transformParameterTToArtifactviewsParameterTView builds a value of type
// *artifactviews.ParameterTView from a value of type *ParameterT.
func transformParameterTToArtifactviewsParameterTView(v *ParameterT) *artifactviews.ParameterTView {
	if v == nil {
		return nil
	}
