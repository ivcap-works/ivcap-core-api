...
// BuildRemoveMetadataRequest instantiates a HTTP request object with method
// and path set to call the "artifact" service "removeMetadata" endpoint
func (c *Client) BuildRemoveMetadataRequest(ctx context.Context, v interface{}) (*http.Request, error) {
	var (
		id     string
		schema string
	)
	{
		p, ok := v.(*artifact.RemoveMetadataPayload)
		if !ok {
			return nil, goahttp.ErrInvalidType("artifact", "removeMetadata", "*artifact.RemoveMetadataPayload", v)
		}
		id = p.ID
		schema = p.Schema
	}
	u := &url.URL{Scheme: c.scheme, Host: c.host, Path: RemoveMetadataArtifactPath(id, schema)}
	req, err := http.NewRequest("DELETE", u.String(), nil)
	if err != nil {
		return nil, goahttp.ErrInvalidURL("artifact", "removeMetadata", u.String(), err)
	}
	if ctx != nil {
		req = req.WithContext(ctx)
	}
...
// EncodeRemoveMetadataRequest returns an encoder for requests sent to the
// artifact removeMetadata server.
func EncodeRemoveMetadataRequest(encoder func(*http.Request) goahttp.Encoder) func(*http.Request, interface{}) error {
	return func(req *http.Request, v interface{}) error {
		p, ok := v.(*artifact.RemoveMetadataPayload)
		if !ok {
			return goahttp.ErrInvalidType("artifact", "removeMetadata", "*artifact.RemoveMetadataPayload", v)
		}
		{
			head := p.JWT
			if !strings.Contains(head, " ") {
...
// DecodeRemoveMetadataResponse returns a decoder for responses returned by the
// artifact removeMetadata endpoint. restoreBody controls whether the response
// body should be restored after having been read.
// DecodeRemoveMetadataResponse may return the following errors:
//   - "invalid-credential" (type *artifact.InvalidCredentialsT): http.StatusBadRequest
//   - "invalid-scopes" (type *artifact.InvalidScopesT): http.StatusForbidden
//   - "not-implemented" (type *artifact.NotImplementedT): http.StatusNotImplemented
//   - "not-found" (type *artifact.ResourceNotFoundT): http.StatusNotFound
//   - "not-authorized" (type *artifact.UnauthorizedT): http.StatusUnauthorized
//   - error: internal error
func DecodeRemoveMetadataResponse(decoder func(*http.Response) goahttp.Decoder, restoreBody bool) func(*http.Response) (interface{}, error) {
	return func(resp *http.Response) (interface{}, error) {
		if restoreBody {
			b, err := io.ReadAll(resp.Body)
			if err != nil {
		...
		case http.StatusNoContent:
			return nil, nil
		case http.StatusBadRequest:
			return nil, NewRemoveMetadataInvalidCredential()
		case http.StatusForbidden:
			var (
				body RemoveMetadataInvalidScopesResponseBody
				err  error
			)
			err = decoder(resp).Decode(&body)
			if err != nil {
				return nil, goahttp.ErrDecodingError("artifact", "removeMetadata", err)
			}
			err = ValidateRemoveMetadataInvalidScopesResponseBody(&body)
			if err != nil {
				return nil, goahttp.ErrValidationError("artifact", "removeMetadata", err)
			}
			return nil, NewRemoveMetadataInvalidScopes(&body)
		case http.StatusNotImplemented:
			var (
				body RemoveMetadataNotImplementedResponseBody
				err  error
			)
			err = decoder(resp).Decode(&body)
			if err != nil {
			...
			err = ValidateRemoveMetadataNotImplementedResponseBody(&body)
			if err != nil {
				return nil, goahttp.ErrValidationError("artifact", "removeMetadata", err)
			}
			return nil, NewRemoveMetadataNotImplemented(&body)
		case http.StatusNotFound:
			var (
				body RemoveMetadataNotFoundResponseBody
				err  error
			)
			err = decoder(resp).Decode(&body)
			if err != nil {
			...
			err = ValidateRemoveMetadataNotFoundResponseBody(&body)
			if err != nil {
				return nil, goahttp.ErrValidationError("artifact", "removeMetadata", err)
			}
			return nil, NewRemoveMetadataNotFound(&body)
		case http.StatusUnauthorized:
			return nil, NewRemoveMetadataNotAuthorized()
		default:
			body, _ := io.ReadAll(resp.Body)
			return nil, goahttp.ErrInvalidResponse("artifact", "removeMetadata", resp.StatusCode, string(body))
		}
	}
}
...
// unmarshalParameterTResponseBodyToArtifactviewsParameterTView builds a value
// of type *artifactviews.ParameterTView from a value of type
// *ParameterTResponseBody.
func unmarshalParameterTResponseBodyToArtifactviewsParameterTView(v *ParameterTResponseBody) *artifactviews.ParameterTView {
	if v == nil {
		return nil
	}
	res := &artifactviews.ParameterTView{
		Name:  v.Name,
		Value: v.Value,
	}
